...
- [17/07 local] Auditoría /fanout (4 ángulos) + Día 1 completo aplicado: countdown al 6/08 (hero kicker + banner), mail/asunto/shares con el deadline, remitente (nombre/ciudad) y provincia persistentes en localStorage, share post-envío, navigator.share, botón "Abriendo…", contacto primero en perfil mobile, link a campaña desde perfil/footer, contraste dictamen AA (#3E6690), **72 páginas prerender s/<slug>.html con tarjeta OG personalizada og/<slug>.jpg** (los shares de perfil apuntan ahí — card de WhatsApp con cara y nombre), sitemap.xml, robots.txt, 404.html, .nojekyll, HTTPS enforced. Tarjetas OG se regeneran con scratchpad/og_senadores.py (Pillow). Los dos PROYECTOS de la auditoría quedaron implementados: (a) panel "tus senadores" embebido en el hero al elegir provincia — caras + "Mandales un mail a los N ahora" (funnel de 1 tap, la provincia persiste); (b) generador de stories 1080×1920 100% client-side (canvas: sol, Didot, countdown, fotos circulares) con `navigator.share({files})` hacia el share sheet (Instagram) y fallback a descarga — botones "Historia para Instagram" en el panel multiplicá post-envío (campaña), en cada perfil (con la cara del senador) y en el footer.
- [17/07] Voz pública sumada: botón "Preguntale su voto en X" en perfiles (tweet pre-armado con handle + #LeyDeTierras + URL prerender), `#/difusion` (kit: texto WhatsApp copiable, placas og/ de los 14 en juego con descargar/mencionar, botón .ics "Agendar el 6 de agosto" con alarma día previo), segundo botón del hero a los 14 indecisos, story copia el link al portapapeles para el sticker de enlace de IG.
